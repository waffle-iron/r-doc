...
class StepsTestSeeder extends Seeder
{
  /**
  ...
  public function run(Faker\Generator $faker)
  {
    ...
    $steps->each(function ($s, $key) use ($faker, $bullets) {
      $s->update(['orderby' => $key + 1]);
      $images = factory(Image::class, $faker->numberBetween(2, 3))->create();
      $images->each(function ($img, $key) use ($s) {
        $img->update(['orderby' => $key + 1]);
        $s->images()->save($img);
      });
      $guide = App\Guide::find(1);
      $guide->steps()->save($s);
